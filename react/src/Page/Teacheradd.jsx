...
const API_BASE = (RAW_BASE || "http://localhost/I-LOVE-MY-JOB-MAIN/server").replace(
  /\/+$/,
  ""
);
...
function Teacheradd() {
  ...
  const fetchTeachers = async () => {
    setIsFetching(true);
    try {
      const { data } = await api.get("/api/GET/get_teachers.php", {
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const { data } = await api.delete("/api/Delete/DeleteTeacher.php", {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        ...
        const { data } = await api.put("/api/UPDATE/UpdateTeacher.php", payload, {
          headers: { "Content-Type": "application/json" },
        });
        ...
        const { data } = await api.post("/api/POST/InsertTeacher.php", formData, {
          headers: { "Content-Type": "application/json" },
        });
                      ...
                      <tr key={t.teacher_id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm font-medium text-gray-900">
                            {formatPrefix(t.prefix)} {t.first_name} {t.last_name}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
    ...
    <div className="bg-white rounded-xl shadow-2xl max-w-md w-full mx-4 p-8">
      ...
      <p className="text-sm text-gray-600 text-center mb-6">
        ...
        <span className="font-semibold text-gray-900">
          {formatPrefix(teacherToDelete.prefix)} {teacherToDelete.first_name}{" "}
          {teacherToDelete.last_name}
        </span>{" "}
